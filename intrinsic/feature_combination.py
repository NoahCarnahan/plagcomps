...
def train(features, cluster_type, atom_type, ntrain, start_doc=0, **cluster_args):
    '''
...
def predict(model, features, cluster_type, atom_type, start_doc, ntest, **cluster_args):
    ...
    '''
    test_matrix, test_actuals = _get_feature_conf_and_actuals(features, cluster_type, atom_type, start_doc, ntest)
    ...
    print 'pct correct:'
    print sum([x == y for x, y in zip(predicted, test_actuals)]) / float(len(predicted))
...
def train_and_predict(features, cluster_type, atom_type, start_doc, ntrain, ntest, **cluster_args):
    ...
    confidences = test(model, features, cluster_type, atom_type, test_start, ntest, **cluster_args)

    return confidences

def _get_feature_conf_and_actuals(features, cluster_type, atom_type, start_doc, n, **cluster_args):
    '''
    ...
    first_training_files = IntrinsicUtility().get_n_training_files(n, first_doc_num=start_doc)
    session = Session()
...
def _test():
    ...
    confs = train_and_test(features, cluster_type, atom_type, start_doc, ntrain, ntest)
