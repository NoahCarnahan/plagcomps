...
class ProcessedDocument(Base):
    ...
    name =              Column(String)
    features =          Column(ARRAY(String)) # http://docs.sqlalchemy.org/en/rel_0_8/core/types.html
    atom_type =         Column(String)
    feature_vectors =   Column(ARRAY(Float)) # Handles multi dimensionality automatically  
...
class ClusteredFeatures(Base):
    ...
    '''
    __table__ = "clustered_features"
    
    # Do we need both document and document_id? Is processed_document created automatically? 
...
class PerformanceEvaluation(Base):
    ...
    __table__ = "performace_evaluations"
